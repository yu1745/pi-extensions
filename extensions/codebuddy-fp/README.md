...
腾讯网关内置了针对“第三方套壳反代给 Claude Code 官方 CLI 使用”的签名级风控拦截：

- **现象**：请求直接被上游拦截，返回 `400`（内部错误码 `code: 11128, msg: "Illegal API invocation from an unapproved channel"` / `请求被安全策略拦截`），pi 表现为 `Error: 400 status code (no body)` 且当前 session 后续所有输入持续报错。
- **触发条件**：对话上下文（包括用户输入、模型历史回复、工具执行输出）中包含完整的 Claude Code 官方系统提示词首句硬编码签名：
  ```
  You are Claude Code, Anthropic's official CLI for Claude.
  ```
- **影响**：日常使用极难触发；但如果你在对话中分析 Claude Code 源码、引用或打印其完整系统提示词签名，该内容会被写入当前 session 的多轮历史并持续上报，导致该会话被腾讯安全策略永久封死。
- **应对**：如果偶发踩中，开启新会话（`/new`）或避免在上下文出现上述完整签名字符串即可。
